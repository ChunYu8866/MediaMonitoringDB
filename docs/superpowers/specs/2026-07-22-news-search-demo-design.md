...
- `q`：必要，正規化後 2–50 個字元。
- `range`：`24h`、`48h` 或 `7d`，預設 `24h`。
- `sources`：可選的來源 ID 清單，只接受白名單值。
- `limit`：1–100，預設 50。
...
```json
{
  "schemaVersion": "2.0.0",
  "query": "台積電",
  "fetchedAt": "2026-07-22T11:00:00Z",
  "cache": "hit",
  "partial": false,
  "sourceStatus": [],
  "items": []
...
每個 item 至少包含 `id`、`sourceId`、`sourceName`、`title`、`excerpt`、`publishedAt`、`url`、`matchedTerms`。不得包含完整文章內容。
...
回應包含 `fetchedAt`、`sourceUrl`、`stale` 及趨勢 items。每個 item 只保存 `title`、`approxTraffic`、`pubDate` 與必要來源連結；第一版不公開第三方新聞圖片。
