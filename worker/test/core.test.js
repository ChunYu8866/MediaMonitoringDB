...
test('parseTrendsRss reads Taiwan Trending Now RSS extension fields', () => {
...
test('normalizeCurrents keeps only the same public news metadata contract', () => {
  const result = normalizeCurrents({
    news: [{ id: 'a', title: '測試新聞', description: '摘要', url: 'https://example.com/a', published: '2026-07-22T08:00:00Z' }],
  });

  assert.deepEqual(Object.keys(result[0]).sort(), ['excerpt', 'id', 'publishedAt', 'sentiment', 'source', 'title', 'url']);
  assert.equal(result[0].source, 'currents');
});
